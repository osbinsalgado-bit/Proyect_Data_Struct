...
export default function OnboardingPage() {
  ...
  const [errors, setErrors] = useState([]); // Nuevo: Para rastrear errores
  ...
  const validateStep = (currentStep) => {
    ...
    if (currentStep === 1) {
      ...
      if (!formData.sedePrincipal.trim()) newErrors.push('sedePrincipal');
      if (!formData.emailSoporte.trim()) newErrors.push('emailSoporte');
      if (formData.dominios.length === 0) newErrors.push('dominios');
    }

    if (currentStep === 2) {
      if (!formData.logoFile) newErrors.push('logoFile');
    }

    if (currentStep === 4) {
      if (!adminData.email.trim()) newErrors.push('adminEmail');
      if (!adminData.password.trim()) newErrors.push('adminPassword');
    }
  ...
  const addDominio = () => {
    ...
    if (d && d.includes('.') && !formData.dominios.includes(d)) {
      ...
      setErrors(errors.filter(e => e !== 'dominios')); // Limpiar error si agrega uno
    }
  };
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      setFormData({ ...formData, logoFile: file, logoPreview: URL.createObjectURL(file) });
      setErrors(errors.filter(e => e !== 'logoFile')); // Limpiar error
    }
  };
  ...
  const handleFinish = async () => {
    ...
    setLoading(true);
    try {
      // A. CERRAR SESIÓN PREVIA (Para evitar conflictos de tokens)
      await signOut(auth);
      ...
      // 1. GUARDAR LA INSTITUCIÓN EN FIRESTORE
      await setDoc(doc(db, "instituciones", instId), {
        nombre: formData.nombre,
        dominioPermitido: formData.dominios,
        logoUrl: logoUrl,
        planSuscripcion: plan,
        temaColorPrincipal: formData.tema.principal, 
        temaColorSecundario: formData.tema.interfaz,
        estado: "activo",
      ...
      const adminPass = adminData.password.trim(); // Limpiamos espacios accidentales en pass
      ...
      try {
        ...
        if (authError.code === 'auth/email-already-in-use') {
          try {
            // Si el correo ya existe, intentamos loguear para confirmar identidad
            const loginCred = await signInWithEmailAndPassword(auth, adminEmail, adminPass);
            uid = loginCred.user.uid;
          } catch (loginErr) {
            // Si falla el login, es que la contraseña realmente no coincide con la base de datos de Firebase
            throw new Error("Contraseña incorrecta. Este correo ya tiene una cuenta en SGL; usa la contraseña de tu cuenta existente para vincular esta nueva institución.");
          }
        } else {
          throw authError;
        }
      ...
      // 3. CREAR PERFIL PARA ESTA INSTITUCIÓN ESPECÍFICA (Multi-Tenancy)
      const profileId = `${uid}_${instId}`;

      await setDoc(doc(db, "usuarios", profileId), {
        uid: uid, 
        email: adminEmail,
      ...
      // 4. ENVÍO DE CORREO DINÁMICO
      const instDataForEmail = {
        nombre: formData.nombre,
        logoUrl: logoUrl,
        temaColorPrincipal: formData.tema.principal,
        temaColorSecundario: formData.tema.interfaz
      ...
      const bodyHtml = `
        <p>¡Hola! Te damos una gran bienvenida a la familia SGL. Tu infraestructura para <b>${formData.nombre}</b> está lista.</p>
        <div style="background-color: ${formData.tema.interfaz}; padding: 20px; border-radius: 15px; border: 1px solid ${formData.tema.principal}33; margin: 20px 0;">
          <p style="margin: 0; color: ${formData.tema.principal}; font-weight: bold;">🚀 Sistema Desplegado</p>
        </div>
        <p>Accede con tu cuenta única para gestionar todas tus instituciones desde un solo lugar.</p>
      `;

      await sendBrandedEmail(adminEmail, `Bienvenido a ${formData.nombre}`, instDataForEmail, bodyHtml);

      localStorage.setItem('institucionActualId', instId);

      // Usa esto para forzar la recarga con el nuevo contexto
      setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 font-sans relative overflow-hidden">
      ...
      <div className="max-w-5xl w-full grid grid-cols-1 lg:grid-cols-12 bg-white rounded-[3.5rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] overflow-hidden z-10 relative border border-white/20">
        ...
        <div className="hidden lg:flex lg:col-span-5 flex-col justify-between p-12 relative transition-all duration-700 border-r border-slate-100 overflow-hidden" 
          ...
          <div className="relative z-10">
            ...
            <div className="bg-white p-3 rounded-[1.8rem] shadow-2xl border border-slate-100 transform hover:scale-[1.02] transition-all">
               ...
               <div className="space-y-6">
                  <div>
                    <h3 className="text-xl font-black text-slate-800 leading-tight tracking-tighter">{formData.nombre || 'Nombre de la Institución'}</h3>
          ...
          <div className="relative group">
            ...
            <div className="relative bg-slate-950/90 backdrop-blur-xl text-white p-7 rounded-[2.2rem] flex items-center justify-between border border-white/10 shadow-2xl">
               <div className="flex items-center space-x-5">
                  <div className={`p-1 rounded-2xl transition-all duration-500 ${plan === 'premium' ? 'bg-amber-400 text-amber-950 shadow-[0_0_25px_rgba(251,191,36,0.4)]' : 'bg-blue-600 text-white shadow-[0_0_25px_rgba(37,99,235,0.4)]'}`}>
        ...
        {/* PANEL DERECHO */}
        <div className="lg:col-span-7 p-8 lg:p-10 bg-white overflow-y-auto max-h-[90vh] custom-scrollbar">
          
          <div className="flex justify-between items-center mb-14">
            <h2 className="text-2xl font-black text-slate-800 uppercase italic leading-none tracking-tighter">Configuración</h2>
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2.5">
                  ...
                  <input type="text" className={`w-full p-3 bg-slate-50 border ${errors.includes('nombre') ? 'border-red-500 ring-2 ring-red-100' : 'border-slate-100'} rounded-[2rem] outline-none font-bold`} placeholder="Ej: Universidad del Norte" value={formData.nombre} onChange={e => {setFormData({...formData, nombre: e.target.value}); setErrors(errors.filter(err => err !== 'nombre'))}} />
                </div>
                <div className="space-y-2.5">
                  <label className="text-[10px] font-black text-slate-400 uppercase ml-5 tracking-widest">Sede / Campus</label>
                  <input type="text" className={`w-full p-3 bg-slate-50 border ${errors.includes('sedePrincipal') ? 'border-red-500 ring-2 ring-red-100' : 'border-slate-100'} rounded-[2rem] outline-none font-bold`} placeholder="Ej: Campus Central" value={formData.sedePrincipal} onChange={e => {setFormData({...formData, sedePrincipal: e.target.value}); setErrors(errors.filter(err => err !== 'sedePrincipal'))}} />
                </div>
              </div>
              
              <div className="space-y-2.5">
                <label className="text-[10px] font-black text-slate-400 uppercase ml-5 tracking-widest">Email de Soporte Oficial</label>
                <input type="email" className={`w-full p-3 bg-slate-50 border ${errors.includes('emailSoporte') ? 'border-red-500 ring-2 ring-red-100' : 'border-slate-100'} rounded-[2rem] outline-none font-bold`} placeholder="soporte@institucion.edu" value={formData.emailSoporte} onChange={e => {setFormData({...formData, emailSoporte: e.target.value}); setErrors(errors.filter(err => err !== 'emailSoporte'))}} />
              </div>

              <div className="space-y-2.5">
                <label className="text-[10px] font-black text-slate-400 uppercase ml-5 tracking-widest">Dominios Oficiales</label>
                <div className="flex space-x-3">
                  <input type="text" className={`flex-1 p-3 bg-slate-50 border ${errors.includes('dominios') && formData.dominios.length === 0 ? 'border-red-500 ring-2 ring-red-100' : 'border-slate-100'} rounded-[2rem] outline-none font-bold`} placeholder="ejemplo.edu" value={formData.currentDominio} onChange={e => setFormData({...formData, currentDominio: e.target.value})} />
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  {formData.dominios.map(d => (
                    <span key={d} className="flex items-center px-6 py-2 bg-slate-950 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest animate-in zoom-in">
              ...
              <button onClick={() => { if(validateStep(1)) setStep(2); }} className="w-full mt-10 bg-slate-950 text-white py-7 rounded-[2.5rem] font-black text-xs uppercase tracking-[0.4em] shadow-2xl flex justify-center items-center hover:bg-blue-600 transition-all">
          ...
          {step === 2 && (
            <div className="space-y-10 animate-in fade-in slide-in-from-right-10">
              <div className={`flex items-center space-x-10 bg-slate-50 p-2 rounded-[3.5rem] border ${errors.includes('logoFile') ? 'border-red-500 ring-2 ring-red-100' : 'border-slate-100'}`}>
              ...
              <div className="flex space-x-6 mt-12">
                ...
                <button onClick={() => { if(validateStep(2)) setStep(3); }} className="flex-1 bg-slate-950 text-white py-7 rounded-[2.5rem] font-black text-xs uppercase tracking-[0.4em] shadow-2xl hover:bg-blue-600 transition-all">Siguiente</button>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-3 animate-in fade-in slide-in-from-right-4">
              <div onClick={() => setPlan('trial')} className={`group p-5 rounded-[3.5rem] border-4 cursor-pointer transition-all ${plan === 'trial' ? 'border-blue-600 bg-blue-50' : 'border-slate-50 opacity-40 hover:opacity-100'}`}>
                 ...
                 <h3 className="font-black text-xl text-slate-800 uppercase italic leading-none">30 Días de Prueba</h3>
                 <p className="text-xs text-slate-500 font-bold mt-2 italic">Acceso completo para que configures todos tus laboratorios.</p>
              </div>

              <div onClick={() => setPlan('premium')} className={`group p-5 rounded-[3.5rem] border-4 cursor-pointer transition-all relative overflow-hidden ${plan === 'premium' ? 'border-amber-400 bg-amber-50 shadow-[0_0_50px_rgba(251,191,36,0.1)]' : 'border-slate-50 opacity-40 hover:opacity-100'}`}>
                 {plan === 'premium' && <div className="absolute top-0 right-0 bg-amber-400 text-amber-900 px-12 py-1.5 rounded-bl-[2.5rem] font-black text-[10px] uppercase">Master SaaS</div>}
                 <div className="flex justify-between items-center mb-6">
                    <div className={`p-4 rounded-3xl shadow-2xl transition-all duration-500 ${plan === 'premium' ? 'bg-amber-400 text-amber-900' : 'bg-slate-100 text-slate-300'}`}>
                        <Crown size={24} fill={plan === 'premium' ? 'currentColor' : 'none'} />
                    </div>
                    <div className="text-right ">
                       <span className="text-2xl font-black text-slate-800">$12 <span className="text-sm line-through text-slate-300 font-bold">$24</span></span>
                       <p className="text-[11px] uppercase text-amber-600 font-black tracking-widest mt-1">Suscripción Anual</p>
                    </div>
                 </div>
                 <h3 className="font-black text-xl text-slate-800 uppercase italic leading-none">Plan Profesional</h3>
          ...
          {step === 4 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
              <input type="email" className={`w-full p-7 bg-slate-50 border ${errors.includes('adminEmail') ? 'border-red-500 ring-2 ring-red-100' : 'border-slate-100'} rounded-[2.5rem] outline-none font-bold`} placeholder="admin@institucion.edu" onChange={e => {setAdminData({...adminData, email: e.target.value}); setErrors(errors.filter(err => err !== 'adminEmail'))}} />
              <input type="password" className={`w-full p-7 bg-slate-50 border ${errors.includes('adminPassword') ? 'border-red-500 ring-2 ring-red-100' : 'border-slate-100'} rounded-[2.5rem] outline-none font-bold`} placeholder="Contraseña Maestra" onChange={e => {setAdminData({...adminData, password: e.target.value}); setErrors(errors.filter(err => err !== 'adminPassword'))}} />
              ...
              <div className="p-8 bg-blue-50 rounded-[3rem] border border-blue-100 flex items-center space-x-6 group overflow-hidden relative">
                 ...
                 <div>
                    ...
                    <p className="text-[11px] text-blue-700 font-bold leading-relaxed mt-1 tracking-tight">Desplegaremos el Sistema Inmediatamente</p>
                 </div>
              </div>
              ...
              <div className="flex space-x-6 mt-12">
                ...
                <button onClick={handleFinish} disabled={loading} className="flex-1 bg-blue-600 text-white py-7 rounded-[2.5rem] font-black text-xs uppercase tracking-widest shadow-2xl hover:bg-blue-700 transition-all active:scale-95">
                  {loading ? <Loader2 className="animate-spin mx-auto" /> : "DESPLEGAR SISTEMA"}
                </button>
              </div>
            </div>
          )}
      ...
      <div className="absolute bottom-2 flex items-center space-x-4 opacity-40">
