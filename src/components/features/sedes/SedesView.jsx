...
import { MapPin, Plus, Trash2, Building2, X, ChevronRight, Map, User } from 'lucide-react'; // Añadí User icon

export default function SedesView({ inst, temaColor, onVerLaboratorios, soloLectura }) {
  ...
  // 2. Cargar Coordinadores de la institución
  useEffect(() => {
    if (!instId) return;
    const q = query(
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-10 duration-700 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {sedes.map(sede => {
          // LÓGICA DE DETECCIÓN: Buscamos qué coordinadores tienen el ID de esta sede en su perfil
          const coordsAsignados = coordinadores.filter(u => u.sedesAsignadas?.includes(sede.id));
            ...
            <div key={sede.id} className="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col group hover:shadow-2xl transition-all duration-500 border-b-[6px]" style={{ borderBottomColor: temaColor }}>
              <div className="p-8 space-y-6">
                <div className="flex justify-between items-start">
                  ...
                  {!soloLectura && (
                    <button onClick={() => { if(window.confirm("¿Eliminar sede?")) deleteDoc(doc(db, "sedes", sede.id)) }} className="p-2 text-red-100 hover:text-red-500 transition-colors"><Trash2 size={18} /></button>
                  )}
                </div>
                <div>
                ...
                <div className="pt-4 border-t border-slate-50">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {coordsAsignados.length > 0 ? (
                      coordsAsignados.map(coord => (
                        <div key={coord.id} className="flex items-center space-x-2 px-3 py-1.5 bg-slate-50 border border-slate-100 rounded-xl">
                          <div className="w-4 h-4 rounded-full bg-blue-500 flex items-center justify-center text-[8px] text-white font-bold">
                            {coord.nombre?.charAt(0)}
                          </div>
